...
float alpha = clamp(max(lineVis * 0.6, combinedPulse * 0.6), 0.0, 1.0);
...
export const GridScan = ({
  ...
  return (
    ...
    >
      {showPreview && (
        <div className="absolute right-3 bottom-3 w-[220px] h-[132px] rounded-lg overflow-hidden border border-white/25 shadow-[0_4px_16px_rgba(0,0,0,0.4)] bg-white text-white text-[12px] leading-[1.2] font-sans pointer-events-none">
          <video
            ref={videoRef}
            muted
            playsInline
          ...
          />
          <div className="absolute left-2 top-2 px-[6px] py-[2px] bg-white/0 rounded-[6px] backdrop-blur-[4px]">
            {enableWebcam
              ? modelsReady
                ? uiFaceActive
                  ? "Face: tracking"
